...
def RentabilidadeRetorno(dados):
    ...
    volatibilidade = pd.DataFrame(retorno_diario.std(), columns=['Volatibilidade'])
    retornos_medios = pd.DataFrame(retorno_diario.mean())
    retornos_medios = retornos_medios.rename(columns={0: 'Retornos'})
...
#     volatibilidade = pd.DataFrame(retorno_diario.std(), columns=['Volatibilidade'])
#     retornos_medios = pd.DataFrame(retorno_diario.mean())
#     retornos_medios = retornos_medios.rename({0: 'Retornos'}, axis=1)
#     risco_retorno = pd.concat([retornos_medios, volatibilidade], axis=1)
